let promptsChart = new CustomChartJs({
    ...
    accuracyChart = new CustomChartJs({
    ...
    tokenChart = new CustomChartJs({
        ...
        options: () => ({
            ...
            data: {
                ...
                datasets: [
                    {
                        data: [82e3, 95e3, 103e3, 112e3, 121500, 135200, 148e3],
...
function generateSmoothData(a, r = 40, t = 5) {
    var e = [r];
    for (let r = 1; r < a; r++) {
        var s = e[r - 1] + (Math.random() * t * 2 - t);
        e.push(Math.round(s));
    }
    return e;
}
function generateHigherData(r, a = [3, 6]) {
    return r.map(
        (r) => r + Math.floor(Math.random() * (a[1] - a[0] + 1)) + a[0]
    );
}
let labels = ["0h", "3h", "6h", "9h", "12h", "15h", "18h", "21h"],
    currentAiUsers = generateSmoothData(8, 45, 4),
    previousAiUsers = generateHigherData(currentAiUsers),
    aiUsersChart = new CustomChartJs({
